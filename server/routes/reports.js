...
const { buildAnnualReportXlsxBuffer }  = require('../services/annualReportXlsx');
...
// ─── Shared: build the annual night-crew workbook ─────────────────────────────
async function buildAnnualWorkbook(year) {
  const report = buildAnnualReport(year);
  const buf    = await buildAnnualReportXlsxBuffer(report);
  const filename = `ASVAC_Night_Crew_${year}.xlsx`;
  return { buf, filename, report };
}

// ─── GET /api/reports/annual?year=YYYY ────────────────────────────────────────
// Defaults to the last full calendar year. Override with ?year=YYYY for any
// past year or the current year (partial).
router.get('/annual', async (req, res) => {
  try {
    ...
    const { buf, filename } = await buildAnnualWorkbook(year);
    res.setHeader('Content-Disposition', `attachment; filename="${filename}"`);
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.send(buf);
  } catch (err) {
...
// ─── POST /api/reports/annual-in-sheets?year=YYYY ─────────────────────────────
// Uploads the annual XLSX to Google Drive (as a native Sheet) and returns the
// URL. Mirrors the open-in-sheets pattern.
router.post('/annual-in-sheets', async (req, res) => {
  const drive = getDriveClient();
  if (!drive) return res.status(401).json(needsAuthResponse());
  ...
  try {
    ...
    console.error('[reports] annual-in-sheets error:', err);
    if (err.code === 401 || err.message?.includes('invalid_grant')) {
      try { fs.unlinkSync(TOKENS_PATH); } catch (_) {}
      return res.status(401).json(needsAuthResponse());
    }
...
// the calendar is edited (swaps) or crew_roster.json is updated.
